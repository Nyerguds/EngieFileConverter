...
namespace EngieFileConverter.UI
{
    public partial class FrmFileConverter : Form
    {
        ...
        private void ReloadUi(Boolean fromNewFile)
        {
            ...
            Int32 bpp = this.m_LoadedFile != null ? this.m_LoadedFile.BitsPerPixel : -1;
            ...
            Int32 bpc = shownFile.BitsPerPixel;
            this.lblValFilename.Text = GeneralUtils.DoubleAmpersands(shownFile.LoadedFileName);
            this.lblValType.Text = GeneralUtils.DoubleAmpersands(shownFile.ShortTypeDescription);
        ...
        private void TsmiSaveRawClick(Object sender, EventArgs e)
        {
            ...
            Object[] arrParams =
                {//Arguments: func returning SupportedFileType, reload as new, reset auto-zoom, process type indication string.
                    new Func<SupportedFileType>(()=> this.SaveRaw(image, sfd.FileName)),
                    false, false, "Saving"
                };
            this.m_ProcessingThread = new Thread(this.ExecuteThreaded);
            this.m_ProcessingThread.Start(arrParams);
        ...
        private void Save(Boolean frames)
        {
            ...
            String filename = FileDialogGenerator.ShowSaveFileFialog(this, selectType, filteredTypes.ToArray(), typeof(FileImagePng), false, true, loadedFile.LoadedFile, out selectedItem);
            ...
            try
            {
                ...
                if (saveOptions != null && saveOptions.Length > 0)
                {
                    ...
                    soi.Name = GeneralUtils.DoubleAmpersands("Extra save options for " + selectedItem.ShortTypeDescription);
                    soi.Properties = saveOptions;
                    FrmOptions frmExtraOpts = new FrmOptions(GetTitle(), soi);
            ...
            Boolean origNames = false;
            if (frames && loadedFile is FileFrames)
            {
                // FileFrames is a special case; it contains the original filenames
                String filenameEx = Path.GetFileNameWithoutExtension(filename) + "-00000" + Path.GetExtension(filename);
                DialogResult dr = MessageBox.Show("Save frames as original internal names?\n\nIf not, names will be generated as \"" + filenameEx + "\"", GetTitle(), MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1);
                origNames = dr == DialogResult.Yes;
            }

            Object[] arrParams =
                {//Arguments: func returning SupportedFileType, reload as new, reset auto-zoom, process type indication string.
                    new Func<SupportedFileType>(()=> this.SaveFile(frames, origNames, loadedFile, selectedItem, filename, saveOptions)),
                    false, false, "Saving"
                };
            this.m_ProcessingThread = new Thread(this.ExecuteThreaded);
            this.m_ProcessingThread.Start(arrParams);
        ...
        private SupportedFileType SaveFile(Boolean frames, Boolean origNames, SupportedFileType loadedFile, SupportedFileType selectedItem, String filename, SaveOption[] saveOptions)
        {
            try
            {
                if (!frames)
                ...
                else
                {
                    ...
                    for (Int32 i = 0; i < loadedFile.Frames.Length; ++i)
                    {
                        ...
                        String framePath = Path.Combine(path, (origNames ? Path.GetFileNameWithoutExtension(frame.LoadedFileName) : (fileName + i.ToString("D5"))) + extension);
        ...
        private void TsmiOpenClick(Object sender, EventArgs e)
        {
            this.SaveFocus(this);
            SupportedFileType selectedItem;
            String filename = FileDialogGenerator.ShowOpenFileFialog(this, null, SupportedFileType.SupportedOpenTypes, this.m_LastOpenedFolder, "images", null, true, out selectedItem);
            if (filename == null)
                return;
        ...
        private void RefreshColorControls()
        {
            ...
            this.palColorViewer.Visible = showPal;
            if (showPal)
        ...
        private void EditColor(Object sender, EventArgs e)
        {
            ...
            Color color = this.palColorViewer.Palette[colIndex];
            this.EditColor(this.palColorViewer, colIndex, color);
        }
        ...
        private void EditColor(PalettePanel palPanel, Int32 colindex, Color color)
        {
            ...
            Color newCol = Color.FromArgb(color.A, cdl.Color);
            this.SetPaletteColor(palPanel, colindex, newCol, loadedFile);
            this.AllowDrop = true;
            this.LoadFocus();
        }
        ...
        private void TsmiImageToFramesClick(Object sender, EventArgs e)
        {
            ...
            Object[] arrParams =
            {//Arguments: func returning SupportedFileType, reload as new, reset auto-zoom, process type indication string.
                new Func<SupportedFileType>(()=> FileFrames.CutImageIntoFrames(image, imagePath, frameWidth, frameHeight, maxFrames, trimColor, trimIndex, matchBpp, matchPalette, false, shownFile.NeedsPalette)),
        ...
        private void TsmiFramesToSingleImageClick(Object sender, EventArgs e)
        {
            ...
            if (highestBpp <= 8)
                so[3] = new SaveOption("BGI", SaveOptionType.Palette, "Background color around frames", highestBpp + "|" + paletteStr, "0");
            else
                so[3] = new SaveOption("BGC", SaveOptionType.Color, "Background color around frames", hasAlpha ? "A" : hasSimpleTrans ? "T" : String.Empty, "#00000000");
            SaveOptionInfo soi = new SaveOptionInfo();
            soi.Name = "Frames to single image";
            soi.Properties = so;
            FrmOptions extraopts;
            try
            {
                extraopts = new FrmOptions(GetTitle(), soi);
            ...
            {
                MessageBox.Show(this, "Error initializing conversion options: " + ex.Message, GetTitle(), MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private void GenerateHeightMap(Boolean selectHeightMap)
        {
            ...
            if (selectHeightMap)
            {
                ...
                String filename = FileDialogGenerator.ShowOpenFileFialog(this, "Select height levels image", new Type[] { typeof(FileImage) }, pngFileName, "images", null, true, out selectedType);
                this.m_LastOpenedFolder = Path.GetDirectoryName(filename);
                if (filename == null)
                    return;
                if (selectedType == null)
        ...
        private void TsmiTo65X65HeightMapClick(Object sender, EventArgs e)
        {
            ...
            Object[] arrParams =
            {//Arguments: func returning SupportedFileType, reload as new, reset auto-zoom, process type indication string.
                new Func<SupportedFileType>(()=> this.Make65x65HeightMap(image, imgFileName)),
        ...
        private void TsmiSplitShadowsClick(Object sender, EventArgs e)
        {
            if (this.m_LoadedFile == null || this.m_LoadedFile.Frames == null || this.m_LoadedFile.Frames.Length == 0)
                return;
            this.SaveFocus(this);
            SaveOption[] so = new SaveOption[1];
            so[0] = new SaveOption("IND", SaveOptionType.Number, "Input shadow index", "0,255", "4");
            SaveOptionInfo soi = new SaveOptionInfo();
            soi.Name = "Input index for shadow:";
            soi.Properties = so;
            try
            {
                FrmOptions opts = new FrmOptions(GetTitle(), soi);
                //opts.Size = opts.MinimumSize;
                opts.Init(soi);
            ...
            Int32 ind;
            Int32.TryParse(SaveOption.GetSaveOptionValue(so, "IND"), out ind);
            
            Object[] arrParams =
            {//Arguments: func returning SupportedFileType, reload as new, reset auto-zoom, process type indication string.
                new Func<SupportedFileType>(()=> FileFramesWwShpTs.SplitShadows(this.m_LoadedFile, (Byte)ind, 1)),
        ...
        private void TsmiCombineShadowsClick(Object sender, EventArgs e)
        {
            ...
            so[0] = new SaveOption("IND", SaveOptionType.Number, "Input shadow index", "0,255", "4");
            SaveOptionInfo soi = new SaveOptionInfo();
            soi.Name = "Output index for shadow:";
            soi.Properties = so;
            try
            {
                FrmOptions opts = new FrmOptions(GetTitle(), soi);
            ...
            Object[] arrParams =
            {//Arguments: func returning SupportedFileType, reload as new, reset auto-zoom, process type indication string.
                new Func<SupportedFileType>(()=> FileFramesWwShpTs.CombineShadows(this.m_LoadedFile, 1, (Byte)ind)),
        ...
        private void TsmiPasteOnFramesClick(Object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                FrmPasteOnFrames pasteBox = new FrmPasteOnFrames(nrOfFrames, maxWidth, maxHeight, this.m_LoadedFile.BitsPerPixel, this.m_LastOpenedFolder);
            ...
            {
                MessageBox.Show(this, ex.Message, GetTitle(), MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void TsmiExtractPalClick(Object sender, EventArgs e)
        {
            ...
            if (shownImage == null)
                return;
            this.SaveFocus(this);
            Int32 bpp = shownImage.BitsPerPixel;
            Color[] pal = shownImage.GetColors();
            Int32 nrOfColors = pal.Count();
            if (nrOfColors == 0 || (bpp != 1 && bpp != 4 && bpp != 8))
                return;
            ColorStatus cs = this.GetColorStatus();
            Int32 fullPal = 1 << bpp;
            ...
            this.ReloadWithDispose(palImage, true, true);
        }
        ...
        private void ImageToPalette(Boolean fourBit)
        {
            ...
            try
            {
                SaveOption[] so = new SaveOption[2];
                so[0] = new SaveOption("CRX", SaveOptionType.Number, "X", "0," + (shownImage.Width - 1), "0");
                so[1] = new SaveOption("CRY", SaveOptionType.Number, "Y", "0," + (shownImage.Height - 1), "0");
                SaveOptionInfo soi = new SaveOptionInfo();
                ...
                Int32 coordX;
                Int32.TryParse(SaveOption.GetSaveOptionValue(so, "CRX"), out coordX);
                Int32 coordY;
                Int32.TryParse(SaveOption.GetSaveOptionValue(so, "CRY"), out coordY);
                Object[] arrParams =
                {
                    //Arguments: func returning SupportedFileType, reload as new, reset auto-zoom, process type indication string.
                    new Func<SupportedFileType>(() => this.ConvertToPalette(shownImage, coordX, coordY, fourBit)), true, true, "Converting to palette"
                };
                this.m_ProcessingThread = new Thread(this.ExecuteThreaded);
                this.m_ProcessingThread.Start(arrParams);
            }
        ...
        private SupportedFileType ConvertToPalette(SupportedFileType file, Int32 x, Int32 y, Boolean fourBit)
        {
            if (file == null || (file.GetBitmap()) == null)
                return null;
            this.SaveFocus(this);
            Int32 palEdge = fourBit ? 4 : 16;
            Int32 palSize = palEdge * palEdge;
            Int32 palStride = fourBit ? palEdge / 2 : palEdge;
            ...
            Int32 palLen = Math.Min(3 * palSize, imageData.Length - startPoint);
            ...
            Byte[] newImageData = Enumerable.Range(0, palSize).Select(b => (Byte)b).ToArray();
            if (fourBit)
                newImageData = ImageUtils.ConvertFrom8Bit(newImageData, palEdge, palEdge, 4, true);
            PixelFormat pf = fourBit ? PixelFormat.Format4bppIndexed : PixelFormat.Format8bppIndexed;
            Bitmap bm = ImageUtils.BuildImage(newImageData, palEdge, palEdge, palStride, pf, col, Color.Black);
        ...
        private void TsmiMatchToPalette_Click(Object sender, EventArgs e)
        {
            ...
            allPalettes.AddRange(this.m_DefaultPalettes);
            allPalettes.AddRange(this.m_ReadPalettes);
            FrmFramesToPal toPal = new FrmFramesToPal(fileToEdit, allPalettes.ToArray());
            DialogResult dr = toPal.ShowDialog();
            this.pzpImage.CustomColors = toPal.CustomColors;
            if (dr != DialogResult.OK)
                return;
            Int32 matchBpp = toPal.MatchBpp;
            Color[] matchPalette = toPal.MatchPalette;
        ...
        private void TsmiExtract4BitPalClick(Object sender, EventArgs e)
        {
            ...
            SaveOption[] so = new SaveOption[1];
            so[0] = new SaveOption("start", SaveOptionType.Number, "Start index", "0," + 240, "0");
            SaveOptionInfo soi = new SaveOptionInfo();
            ...
            try
            {
                ...
                {
                    Int32 startIndex;
                    Int32.TryParse(SaveOption.GetSaveOptionValue(so, "start"), out startIndex);
            ...
            FrmPalette palFrm = new FrmPalette(shownFile.GetColors(), false, ColorSelMode.Multi);
            palFrm.SelectedIndices = selectedIndices;
            palFrm.Text = "Select 16 colors";
            if (palFrm.ShowDialog(this) != DialogResult.OK)
                return;
            Color[] col = palFrm.GetSelectedColors();
        ...
        {
            this.SaveFocus(this);
            FrmManagePalettes palSave = new FrmManagePalettes(fourBit ? 4 : 8);
            palSave.Icon = this.Icon;
            palSave.Title = GetTitle();
        ...
        /// boolean 'reload as new',
        /// boolean 'reset auto-zoom', 
        /// and a string to indicate the process type being executed (eg. "Saving").
        /// </summary>
        /// <param name="parameters">Array of Object, containing 4 items: func returning SupportedFileType, boolean 'reload as new', boolean 'reset auto-zoom', string to indicate the process type being executed (eg. "Saving").</param>
        private void ExecuteThreaded(Object parameters)
        {
            Object[] arrParams = parameters as Object[];
            Func<SupportedFileType> func;
            if (arrParams == null || arrParams.Length < 4 || (func = arrParams[0] as Func<SupportedFileType>) == null || !(arrParams[1] is Boolean) || !(arrParams[2] is Boolean))
            {

...
namespace Nyerguds.FileData.Epic
{
    public class JazzRleCompression : RleImplementation<JazzRleCompression>
    {
        ...
        /// <returns>A byte array of the given output size, filled with the decompressed data.</returns>
        public static Byte[] RleDecodeJazz(Byte[] buffer, UInt32? startOffset, Boolean hasHeader, UInt32? decompressedSize, Boolean abortOnError, out Int32 actualCompressedSize)
        {
            ...
            if (hasHeader)
            {
                if (buffer.Length < 2)
                    return null; // Error.
                compressedSize = (Int32)ArrayUtils.ReadIntFromByteArray(buffer, (Int32)usableStartOffset, 2, true);
                // Add read value size and start
                usableStartOffset += 2;
                endOffset = (UInt32)compressedSize + usableStartOffset;
            ...
            actualCompressedSize = rle.LastEndPoint == -1 ? compressedSize : (rle.LastEndPoint - (Int32)usableStartOffset);
            return retSize == -1 ? null : bufferOut;
        }

        protected Boolean AbortOnError { get; set; }
        protected Int32 LastEndPoint { get; set; }
        ...
        protected override Boolean GetCode(Byte[] buffer, ref UInt32 inPtr, ref UInt32 bufferEnd, out Boolean isRepeat, out UInt32 amount)
        {
            ...
            if (success && amount == 0)
            {
                ...
                amount = 1;
                bufferEnd = inPtr + 1;
                this.LastEndPoint = (Int32) bufferEnd;
            }
            return success;
        }

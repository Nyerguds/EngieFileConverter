...
namespace CnC64FileConverter.Domain.ImageFile
{
    public class FilePaletteN64 : N64FileType
    {
        ...
        public override void LoadImage(Byte[] fileData)
        {
            ...
            if (len % 3 != 0)
                throw new FileTypeLoadException("Incorrect file size: not a multiple of 3.");
            // Test on full 16-colour lines
            if (len % 48 != 0)
            ...
            try
            {
                palette = ColorUtils.ReadPaletteFile(fileData, false);
            }
            ...
            this.loadedImage = ImageUtils.BuildImage(imageData, 16, 16, 16, PixelFormat.Format8bppIndexed, palette, Color.Empty);
        }
        ...
        protected void SaveAsThis(N64FileType fileToSave, String savePath, Boolean expandToFullSize)
        {
            ...
            ColorUtils.WritePaletteFile(palEntries, savePath, expandToFullSize);
        }
    }
